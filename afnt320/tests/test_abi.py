...
def equates() -> dict[str, int]:
    ...
    for line in (ROOT / "afnt320.inc").read_text().splitlines():
...
class AbiTests(unittest.TestCase):
    def test_public_constants(self) -> None:
        values = equates()
        self.assertEqual([0, 1, 2, 3, 4, 5], [values[f"AFNT320_{name}"] for name in (
            "INIT", "FREE", "FNSTYLE", "APRINT", "SET_WINDOW", "SET_TARGET")])
        self.assertEqual([0, 1, 2, 3], [values[f"AFNT320_TARGET_{name}"] for name in (
            "BUF0", "BUF1", "FRONT", "BACK")])
        self.assertEqual(0x10, values["AFNT320_ERR_ARGUMENT"])
        self.assertEqual(0x12, values["AFNT320_ERR_WINDOW"])

    def test_l0_version_and_entry_offsets(self) -> None:
        raw = (ROOT / "build/AFNT320.DLL").read_bytes()
        self.assertEqual(b"L0", raw[:2])
        self.assertEqual(0x0105, int.from_bytes(raw[14:16], "little"))
        code = decode_code(ROOT / "build/AFNT320.DLL")
        self.assertEqual(bytes([0xC3]) * 6, bytes(code[32 + 3 * i] for i in range(6)))

    def test_shared_target_and_runtime_are_used(self) -> None:
        source = (ROOT / "afnt320.asm").read_text()

# GFX320 hardware benchmark

Русская версия: [BENCHMARK.ru.md](BENCHMARK.ru.md).

## Purpose

The benchmark answers one question: how long do real GFX320 operations take
...
Build with `make -C gfx320 benchmark` and place `build/GFXBENCH.EXE` beside
the matching `GFX320.DLL`. Unlike `GFX320.EXE`, the benchmark build skips
...
batch duration in milliseconds**; bars clamp at the full 320-pixel width
(≥ 320 ms):

1. red — 8 full-screen clears;
2. green — 128 opaque 16×16 tiles;

...
static char dialog_title[] = "WIN320 C dialog";
...
static win_item_t items[6];
static win_window_t window = {
    32, 20, 256, 216, 0xff, WIN_WND_CLOSE, 6, 2, 0, 0, 0xff, 0xff, 0
};
...
static void init_dialog(void) {
    window.title = (win_ptr_t)dialog_title;
    ...
    set_item(0, WIN_T_EDIT, WIN_IT_HIT | WIN_IT_FOCUSABLE, 10,
             (win_ptr_t)&edit);
    set_item(1, WIN_T_LISTBOX, WIN_IT_HIT | WIN_IT_FOCUSABLE, 11,
             (win_ptr_t)&listbox);
    set_item(2, WIN_T_SCROLLBAR, WIN_IT_HIT | WIN_IT_REPEAT, 12,
             (win_ptr_t)&scrollbar);
    set_item(3, WIN_T_BUTTON, WIN_IT_HIT | WIN_IT_FOCUSABLE, 1,
             (win_ptr_t)&ok);
    set_item(4, WIN_T_BUTTON, WIN_IT_HIT | WIN_IT_FOCUSABLE, 2,
             (win_ptr_t)&cancel);
    set_item(5, WIN_T_LABEL, 0, 0xff, (win_ptr_t)&status);
...
int main(void) {
    ...
    for (;;) {
        ...
        if (track.event == WIN_EV_CLOSE || track.id == 2 ||
            (track.event == WIN_EV_KEY && track.key_ascii == 27))
            break;
        if (track.id == 1 || track.id == 11) {
            status.text = (win_ptr_t)status_changed;
            items[5].flags |= WIN_IT_DIRTY;
